...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        exports['qb-target']:AddBoxZone(stashTargetBoxID, coords, 1.5, 1.5, {
            ...
            options = {{
                type = "client",
                event = "qb-hayesmechanicjob:client:target:OpenStash",
                label = "Open Stash",
            }},
            distance = 2.0
        })
    ...
    local OpenMenu = {
        {
            ...
            params = {
                event = "qb-hayesmechanicjob:client:UnattachVehicle",
            }
        }, {
            header = "Check Status",
            txt = "Check Vehicle Status",
            params = {
                event = "qb-hayesmechanicjob:client:CheckStatus",
                args = {
                    number = 1,
                }
            }
            ...
            params = {
                event = "qb-hayesmechanicjob:client:PartsMenu",
                args = {
                    number = 1,
                }
            }
            ...
            txt = "",
            params = {
                event = "qb-hayesmechanicjob:client:target:CloseMenu",
            }
        },
    }
        ...
        for k,v in pairs(Config.ValuesLabels) do
            if math.ceil(VehicleStatus[plate][k]) ~= Config.MaxStatusValues[k] then
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    params = {
                        event = "qb-hayesmechanicjob:client:PartMenu",
                        args = {
                            name = v,
                            parts = k
                        }
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    txt = "Status: " .. percentage .. ".0% / 100.0%",
                    params = {
                        event = "qb-hayesmechanicjob:client:NoDamage",
                    }
                }
            end                               
        end
...
local function PartMenu(data)
    ...
    local TestMenu1 = {
        ...
        },
        {
            ...
            params = {
                event = "qb-hayesmechanicjob:client:RepairPart",
                args = {
                    part = part,
                }
            }
...
local function VehicleList()
    ...
    for k,v in pairs(Config.Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "Vehicle: "..v.."",
            params = {
                event = "qb-hayesmechanicjob:client:SpawnListVehicle",
                args = {
                    headername = v,
                    spawnName = k
                }
    ...
    QBCore.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            QBCore.Functions.Progressbar("repair_part", "Repairing " ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                TriggerServerEvent('qb-inventory:server:SaveStashItems', "hayesmechanicstash", StashItems)
...
RegisterNetEvent("qb-hayesmechanicjob:client:UnattachVehicle",function(data)
    UnattachVehicle()
end)

RegisterNetEvent("qb-hayesmechanicjob:client:PartsMenu",function(data)
    PartsMenu()
end)

RegisterNetEvent("qb-hayesmechanicjob:client:PartMenu",function(data)
    PartMenu(data)
end)

RegisterNetEvent("qb-hayesmechanicjob:client:NoDamage",function(data)
    NoDamage()
end)

RegisterNetEvent("qb-hayesmechanicjob:client:CheckStatus",function(data)
    CheckStatus()
end)

RegisterNetEvent("qb-hayesmechanicjob:client:SpawnListVehicle",function(data)
    local vehicleSpawnName = data.spawnName
    SpawnListVehicle(vehicleSpawnName)
end)

RegisterNetEvent("qb-hayesmechanicjob:client:RepairPart",function(data)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.onduty then
            if PlayerData.job.name == "hayes" then
...
RegisterNetEvent('qb-hayesmechanicjob:client:target:OpenStash', function ()
    TriggerEvent("inventory:client:SetCurrentStash", "hayesmechanicstash")
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "hayesmechanicstash", {
        maxweight = 4000000,
        slots = 500,
    })
end)

RegisterNetEvent('qb-hayesmechanicjob:client:target:CloseMenu', function () 
    DestroyVehiclePlateZone(ClosestPlate)
    RegisterVehiclePlateZone(ClosestPlate, Config.Plates[ClosestPlate])
...
CreateThread(function ()
    ...
    AddTextComponentSubstringPlayerName("Hayes Mechanic")
        ...
        if PlayerJob.name == "hayes" then
            ...
            if onDuty then
                if isInsideStashZone then
                    wait = 0
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent("qb-hayesmechanicjob:client:target:OpenStash")
                    end
                end
